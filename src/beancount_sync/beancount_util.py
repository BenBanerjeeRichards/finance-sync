...
def transactions_equal(a: Transaction, b: Transaction) -> bool:
  ...
  def clean_transaction(old: Transaction) -> Transaction:
    return Transaction(date=old.date, flag=old.flag, payee='' if old.payee is None else old.payee, narration='' if old.narration is None else old.narration, tags=old.tags, links=old.links, postings=[clean_posting(p) for p in old.postings], meta=None)
