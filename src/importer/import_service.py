...
class ImportService:
    ...
    @staticmethod
    def update_monzo_tokens(session, client_id: str, access_token: str, refresh_token: str):
    ...
    @staticmethod
    def get_monzo_config(session, client_id: str) -> MonzoImportIntegrationDto:
    ...
    @staticmethod
    def get_monzo_configs(session) -> list[MonzoImportIntegrationDto]:
    ...
    @staticmethod
    def get_gc_configs(session) -> list[GcImportIntegrationDto]:
    ...
    @staticmethod
    def update_santander_req_date(session, secret_id: str, dt: datetime.datetime):
    ...
    @staticmethod
    def get_monzo_config_by_id(session, import_id: uuid.UUID) -> MonzoImportIntegrationDto:
    ...
    @staticmethod
    def get_gc_config_by_id(session, import_id: uuid.UUID) -> GcImportIntegrationDto:
    ...
    @staticmethod
    def update_monzo_config(session, import_id: uuid.UUID, cash_account_id: uuid.UUID | None,
    ...
    @staticmethod
    def update_gc_config(session, import_id: uuid.UUID, cash_account_id: uuid.UUID | None,
                         default_income_account_id: uuid.UUID | None,
                         default_expense_account_id: uuid.UUID | None):
        ImportService._validate_accounts_exist(
            session, [cash_account_id, default_income_account_id, default_expense_account_id])
    ...
    @staticmethod
    def get_import_config(session, import_id: uuid.UUID) -> tuple[Literal["monzo", "gc"], MonzoImportIntegrationDto | GcImportIntegrationDto]:
        kind = ImportService.get_import_rule_type(session, import_id)
        if kind == "monzo":
            return kind, ImportService.get_monzo_config_by_id(session, import_id)
        if kind == "gc":
            return kind, ImportService.get_gc_config_by_id(session, import_id)
        raise ValueError(f"No import configuration found for id {import_id}")

    @staticmethod
    def update_import_config(session, import_id: uuid.UUID, cash_account_id: uuid.UUID | None,
                             default_income_account_id: uuid.UUID | None,
                             default_expense_account_id: uuid.UUID | None
                             ) -> tuple[Literal["monzo", "gc"], MonzoImportIntegrationDto | GcImportIntegrationDto]:
        kind = ImportService.get_import_rule_type(session, import_id)
        if kind == "monzo":
            ImportService.update_monzo_config(session, import_id, cash_account_id, default_income_account_id,
                                              default_expense_account_id)
            return kind, ImportService.get_monzo_config_by_id(session, import_id)
        if kind == "gc":
            ImportService.update_gc_config(session, import_id, cash_account_id, default_income_account_id,
                                           default_expense_account_id)
            return kind, ImportService.get_gc_config_by_id(session, import_id)
        raise ValueError(f"No import configuration found for id {import_id}")

    def get_monzo_import_rules(self, session, import_integration_id: uuid.UUID) -> list[MonzoImportRuleDto]:
    ...
    @staticmethod
    def get_gc_import_rules(session, import_integration_id: uuid.UUID) -> list[GcImportRuleDto]:
    ...
    @staticmethod
    def upsert_gc_import_rules(session, import_integration_id: uuid.UUID, rules: list[GcImportRuleDto]):
    ...
    @staticmethod
    def upsert_monzo_import_rules(session, import_integration_id: uuid.UUID, rules: list[MonzoImportRuleDto]):
    ...
    @staticmethod
    def delete_monzo_import_rule(session, rule_id: uuid.UUID):
        stmt = delete(MonzoImportRule).where(MonzoImportRule.id == rule_id)
        session.execute(stmt)

    @staticmethod
    def delete_gc_import_rule(session, rule_id: uuid.UUID):
        stmt = delete(GoCardlessImportRule).where(GoCardlessImportRule.id == rule_id)
        session.execute(stmt)

    def get_import_rules(self, session, import_id: uuid.UUID) -> tuple[Literal["monzo", "gc"], list[MonzoImportRuleDto] | list[GcImportRuleDto]]:
        kind = ImportService.get_import_rule_type(session, import_id)
        if kind == "monzo":
            return kind, self.get_monzo_import_rules(session, import_id)
        if kind == "gc":
            return kind, self.get_gc_import_rules(session, import_id)
        raise ValueError(f"No import rules found for id {import_id}")

    @staticmethod
    def delete_import_rule(session, import_id: uuid.UUID, rule_id: uuid.UUID):
        kind = ImportService.get_import_rule_type(session, import_id)
        if kind == "monzo":
            ImportService.delete_monzo_import_rule(session, rule_id)
        elif kind == "gc":
            ImportService.delete_gc_import_rule(session, rule_id)

    @staticmethod
    def get_import_rule_type(session, import_id: uuid.UUID) -> Literal["gc", "monzo"] | None:
        q = select(MonzoImportIntegration).where(MonzoImportIntegration.id == import_id)
        res = session.execute(q).scalar_one_or_none()
        if res:
            return "monzo"
