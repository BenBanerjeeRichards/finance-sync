...
class PosterConfigService:
    ...
    @staticmethod
    def list_configs(session, poster_type: str | None = None) -> list[PosterConfigDto]:
    ...
    @staticmethod
    def get_config(session, config_id: uuid.UUID) -> PosterConfigDto:
    ...
    @staticmethod
    def create_config(session, poster_type: str, name: str, config: dict, enabled: bool = True) -> PosterConfigDto:
    ...
    @staticmethod
    def update_config(session, config_id: uuid.UUID, name: str | None = None, config: dict | None = None,
    ...
    @staticmethod
    def delete_config(session, config_id: uuid.UUID) -> None:
        session.execute(delete(PosterConfig).where(PosterConfig.id == config_id))

    @staticmethod
    def get_energy_config(session) -> EnergyConfig | None:
        configs = PosterConfigService.list_configs(session, "energy")
    ...
    @staticmethod
    def get_accrual_configs(session) -> list[AccrualConfig]:
        configs = PosterConfigService.list_configs(session, "accrual")
        return [AccrualConfig(**c.config) for c in configs if c.enabled]

    @staticmethod
    def get_mortgage_configs(session) -> list[MortgageConfig]:
        configs = PosterConfigService.list_configs(session, "mortgage")
